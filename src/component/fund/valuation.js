...
class Valuation extends Component {
    ...
    componentDidMount(){
        ...
        // document.title = this.props.match.params.fundName +'（'+this.props.match.params.fundId + '）';
        let fundId = this.props.match.params.fundId;
    ...
    fetch208(fundId){
        ...
        fetch(host+"F208",{
            ...
            if(res.respCode === '00' &&res.fundEvalList != null){
                ...
                let dataXX =[]
                let startValuation = (res.startValuation === null||parseInt(res.startValuation)===0)?res.preValuation:res.startValuation
    ...
    render() {
        console.log(this.state.fundEval.preValuation)
        // console.log(this.state.rtChg)
        return (
            <div className="valuation">
                { this.state.loading&&<div className="load">数据加载中...</div>}
                {
                    this.state.f208&&<div className="blueHead">
                ...
                {
                    this.state.f208&&this.state.switch&&<div className="fourBg">
                        {
                ...
                {
                    this.state.f208&&<div className="realWrap bgfff mgt10">
                ...
                {
                    this.state.f205&&<div className="tableWrap bgfff mgt10">
                        <div className="th">
                            <span>股票持仓（前十）</span>
                            <span>占比</span>
                            <span className="width30p">持仓市值</span>
                            {/*<span>最新价</span>*/}
                            {/*<span>涨跌幅</span>*/}
                        </div>
                        {
                            this.state.fundTen.map((value,index)=>{
                                return <div className="tbody" key={index}>
                                    <span>{value.sholding2}({value.esymbol})</span>
                                    <span>{value.sholding7}%</span>
                                    <span className="width30p">{value.sholding5>10000000?(value.sholding5/100000000).toFixed(2)+"亿":(value.sholding5/10000).toFixed(0)+"万"}</span>
                                    {/*<span className={value.isRise ? 'red':'green'}>{value.newPrice}</span>*/}
                                    {/*<span className={value.isRise ? 'red':'green'}>*/}
                                    {/*{value.isRise ? '+':'-'}*/}
                                    {/*{value.priceL}%*/}
                                    {/*</span>*/}
                                </div>
                        ...
                        <div className="holdRate">
                            <div className="big blue">
                                {this.state.amountPer}%
                            </div>
                            <p className="small">前十持仓占比总计</p>
                            <p className="small">(截止日期: {this.state.newDate})</p>
                        </div>
                    </div>
                }
            </div>
