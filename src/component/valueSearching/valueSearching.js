...
class ValueSearching extends Component {
    constructor(props){
        super(props)
        this.state={
            show:1,
    ...
    onPress(data,name){
        let history = localStorage.historyData2?localStorage.getItem("historyData2"):'';
        let item = data + name;
        console.log(item)
        let hisData = localStorage.historyData2?history +','+item:item
        localStorage.setItem('historyData2',hisData)
        //this.props.history.push(`/valuation/${data}/${name}`);
        this.props.history.push(`/regroupFund/${data}`);
    }
