...
import ProgroupFund from './progressFund/progressFund'
...
class Valuation extends Component {
    ...
    renderDetail() {
        ...
        return (
            <div>
                <ProgroupFund onClickshow={this.showmodal.bind(this)}></ProgroupFund>
            </div>
        )
    }
    clickShow(data) {
    ...
    getOption = () => {
        let that = this;
        let ras = Math.min.apply(null, that.state.priceL).toFixed(2)
        let rab = Math.max.apply(null, that.state.priceL).toFixed(2)
        let raa
        ...
        let min = Math.min.apply(null, that.state.fundE)
        let max = Math.max.apply(null, that.state.fundE)
        let realStartVa = (that.state.fundEval.startValuation === null || parseFloat(that.state.fundEval.startValuation) === 0) ? that.state.fundEval.preValuation : that.state.fundEval.startValuation;
            ...
            series: [
                {
                    ...
                    markPoint: {
                        ...
                        data: [
                            {
                                name: '某个坐标',
                                coord: [120, that.state.fundE[120]]
                            }
                        ]
                    },
                    data: that.state.fundE
                },
                {
                    name: '股价基线',
                    type: 'line',
