...
let timer = null
class RankSearching extends Component {
    constructor(props){
        super(props)
        this.state={
            show:1,
    ...
    onPress(data,name){
        let history = localStorage.historyData3?localStorage.getItem("historyData3"):'';
        console.log(history)
        let item = data + name;
        let hisData = localStorage.historyData3?history +','+item:item
        localStorage.setItem('historyData3',hisData)
        // this.props.history.push({pathname: "/result", state: {fundId:data}})
